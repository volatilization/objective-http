...
describe('OutputResponse', () => {
    ...
    describe('update', () => {
        ...
        test('should be equal before and after update', () => {
            const beforeUpdateResultedOutputStream = new OutputResponse(testOptions).flush();
            const afterUpdateResultedOutputStream = new OutputResponse().update(testOptions).flush();

            assert.deepStrictEqual(beforeUpdateResultedOutputStream,
                new Response(testOptions.body, {
                    status: testOptions.statusCode,
                    headers: testOptions.headers
                }));
            assert.deepStrictEqual(beforeUpdateResultedOutputStream, afterUpdateResultedOutputStream);
        });

        test('should not be updated by null or undefined option param', () => {
            const beforeUpdateResultedOutputStream = new OutputResponse(testOptions).flush();
            const afterUpdateResultedOutputStream = new OutputResponse(testOptions)
